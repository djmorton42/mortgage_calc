...
class CalculationProperties(object):
    def __init__(self, start_date, opening_balance, interest_rate, 
        monthly_payment, number_of_months, compounding_months):
    ...
    def effective_monthly_interest_rate(self):
        return ((self.effective_annual_interest_rate + 1) 
            ** (Decimal(1)/12)) - 1

    def replace_with_args(self, args):
        if args["startdate"] is not None:
            self.start_date = datetime.strptime(args["startdate"], 
                "%Y-%m-%d")
